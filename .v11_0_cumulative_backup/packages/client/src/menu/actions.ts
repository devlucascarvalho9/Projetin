...
export default class Actions extends Menu {
    ...
    public override show(
        ...
        // ARPG rarity/item-level metadata is stored in reserved enchantment 9900.
        const meta = enchantments[9900];
        const rarity = meta?.rarity || 'normal';
        const rarityNames: Record<string, string> = { normal: 'Normal', magic: 'Mágico', rare: 'Raro', unique: 'Único' };
        const rarityColours: Record<string, string> = { normal: '#dedede', magic: '#6f9cff', rare: '#f0cf62', unique: '#d88a3d' };
        const rarityAffixLimits: Record<string, { prefixes: number; suffixes: number }> = {
            normal: { prefixes: 0, suffixes: 0 },
            magic: { prefixes: 1, suffixes: 1 },
            rare: { prefixes: 3, suffixes: 3 },
            unique: { prefixes: 3, suffixes: 3 }
        };
        this.name.innerHTML = meta?.source === 'arpg-meta'
            ? `<span style="color:${rarityColours[rarity] || rarityColours.normal};font-weight:bold">${name}</span><br><small style="color:#aaa">${rarityNames[rarity] || 'Normal'} • Item Level ${meta.itemLevel || meta.level || 1}${meta.mapTier ? ` • Map T${meta.mapTier}` : ''}</small>`
            : name;
        ...
        if (itemDescription) this.description.innerHTML = itemDescription;
            ...
            let attack = document.createElement('div'),
                ...
                arpgDiv = document.createElement('div');
            ...
            arpgDiv.id = 'action-description-arpg';

            if (meta?.source === 'arpg-meta') {
                const values = Object.values(enchantments).filter((entry) => ['essence', 'arpg', 'unique'].includes(entry.source || ''));
                const prefixes = values.filter((entry) => entry.affixType === 'prefix').length;
                const suffixes = values.filter((entry) => entry.affixType === 'suffix').length;
                const limits = rarityAffixLimits[rarity] || rarityAffixLimits.normal;
                arpgDiv.innerHTML = `<div style="margin:0 0 8px;padding:6px;border:1px solid #4b4335"><b style="color:${rarityColours[rarity] || '#ddd'}">${rarityNames[rarity] || 'Normal'}</b> • ilvl ${meta.itemLevel || meta.level || 1}<br><small>Prefixos ${prefixes}/${limits.prefixes} • Sufixos ${suffixes}/${limits.suffixes}</small></div>`;
            }
            ...
            if (Object.keys(enchantments).length > 0) {
                ...
                for (let key in enchantments) {
                    const data = enchantments[key];
                    if (data.source === 'arpg-meta') continue;
                    if (['essence', 'arpg', 'unique'].includes(data.source || '')) {
                        const kind = data.affixType === 'prefix' ? 'P' : data.affixType === 'suffix' ? 'S' : 'A';
                        enchantmentsDiv.innerHTML += `<span style="color:#d9aa4f">[${kind}] T${data.tier || data.level}</span> ${data.label || data.stat || 'Atributo'} <br>`;
                        continue;
                    }

                    let enchantment = Modules.Enchantment[key];
                    enchantmentsDiv.innerHTML += `${enchantment || 'Enchant'}: ${data.level} <br>`;
                }
            }

            this.description.append(arpgDiv, attack, defense, bonusesDiv, enchantmentsDiv);
        }
