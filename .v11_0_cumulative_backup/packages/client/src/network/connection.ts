...
export default class Connection {
    ...
    public constructor(private game: Game) {
        ...
        this.messages.onTalent(this.handleTalent.bind(this));
    }
    ...
    private handleCombat(opcode: Opcodes.Combat, info: CombatPacketData): void {
        ...
        // Additional ARPG-style combat feedback layer. This is purely visual;
        // damage remains authoritative on the server.
        if (info.hit.damage > 0) this.game.combatVfx.onHit(attacker, target, info.hit);
        ...
        if (currentPlayerAttacker && info.hit.damage > 0) window.dispatchEvent(new CustomEvent('kaetram-player-attack'));
        ...
        if (currentPlayerTarget && info.hit.damage > 0) window.dispatchEvent(new CustomEvent('kaetram-player-hit'));
    ...
    private handleCommand(info: CommandPacketData): void {
        if (info.command.startsWith('arpg-settings:')) {
            try {
                const settings = JSON.parse(info.command.slice('arpg-settings:'.length));
                (window as any).__kaetramArpgSettings = settings;
                window.dispatchEvent(new CustomEvent('kaetram-arpg-settings', { detail: settings }));
            } catch (error) {
                console.warn('Could not parse ARPG settings.', error);
            }
            return;
        }
    ...
    private handleEnchant(opcode: Opcodes.Enchant, info: EnchantPacketData): void {
        switch (opcode) {
            ...
            case Opcodes.Enchant.EssenceCancel: {
                this.game.handleEssenceForgePacket(opcode, info);
                return;
            }
        }
    }
    ...
    /** Synchronizes the Warrior passive tree with the game/iframe viewer. */
    private handleTalent(_opcode: Opcodes.Talent, data: TalentPacketData): void {
        this.game.updateImportedTalentState(data);
    }
