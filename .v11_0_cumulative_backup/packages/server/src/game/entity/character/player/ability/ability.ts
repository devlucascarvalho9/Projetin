...
export default class Ability {
    ...
    public activate(player: Player): boolean {
        ...
        const supportModifiers = player.getArpgAbilityModifiers(this.key);
        cooldown = Math.max(250, Math.round(cooldown * supportModifiers.cooldownMultiplier));
        mana = Math.max(1, Math.ceil(mana * supportModifiers.manaMultiplier));
