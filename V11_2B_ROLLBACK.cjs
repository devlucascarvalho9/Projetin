...
if(!fs.existsSync(statePath)){console.error('ERRO: estado V11.2B nao encontrado.');process.exit(1);}
const state=JSON.parse(fs.readFileSync(statePath,'utf8')),backupRoot=path.join(root,state.backupRel);let restored=0;
...
fs.rmSync(statePath,{force:true});console.log(`Rollback V11.2B concluido: ${restored} entradas restauradas/removidas.`);
