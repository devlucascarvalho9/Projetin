...
export default class Commands {
    ...
    private async handlePlayerCommands(command: string, blocks: string[]): Promise<void> {
        switch (command) {
            ...
            case 'city':
            case 'cidade2': {
                // V11.1 structured starter city. It lives in rows appended below the
                // original world, so there is literally no legacy terrain underneath it.
                this.player.teleport(88, 1081, true, false, true);
                return this.player.notify('Cidade Inicial Estruturada V11.1 — casas, fonte, rios e estruturas possuem colisao real.');
            }

            case 'cidadeantiga':
            case 'oldcity': {
                this.player.teleport(191, 160, true, false, true);
                return this.player.notify('Teleportado para a cidade animada antiga (V10.x).');
            }
            ...
            case 'teleportes':
            case 'tps': {
                this.player.notify('/cidade — Cidade Inicial Estruturada V11.1');
                this.player.notify('/dungeon — Cripta procedural V11.0 / Crypt Lord');
                this.player.notify('/cidadeantiga — cidade animada anterior, mantida para comparacao');
            ...
            case 'supportlist': {
                const settings = this.player.arpgSettings.supports;
                this.player.syncArpgSettings();
                return this.player.notify(
                    `Supports — Cleave: ${settings.cleave.join(', ') || 'nenhum'} | Whirlwind: ${settings.whirlwind.join(', ') || 'nenhum'} | Warcry: ${settings.warcry.join(', ') || 'nenhum'}`
                );
            }
            ...
            case 'giveitem': {
                ...
                this.world.entities.spawnItem(key, this.player.x, this.player.y, true, count, enchantments, this.player.username);
                return this.player.notify(`Criado: ${item.name || key} x${count} • ilvl ${itemLevel}.`);
            }
            ...
            case 'devstats': {
                ...
                const hp = this.player.debugMaxHitPoints > 0 ? this.player.debugMaxHitPoints : this.player.hitPoints.getMaxHitPoints();
                return this.player.notify(`DEV — LV ${lv} | STR ${str} | HP ${hp} | ATK SPD ${Math.round(this.player.debugAttackSpeedMultiplier * 100)}% | MOVE SPD ${Math.round(this.player.debugMovementSpeedMultiplier * 100)}%`);
            }
            ...
            case 'devset': {
                ...
                if (!Number.isFinite(raw)) return this.player.notify('Uso: /devset [level|str|hp|attackspeed|movespeed] <valor> ou /devset reset');

                switch (stat) {
                    case 'level':
                    case 'lv':
                        this.player.debugCombatLevel = Math.max(1, Math.min(999, Math.floor(raw)));
                        this.player.skills.sync();
                        break;
                    case 'str':
                    case 'strength':
                        this.player.debugStrengthLevel = Math.max(1, Math.min(999, Math.floor(raw)));
                        break;
                    case 'hp':
                    case 'life':
                    ...
                    case 'attackspeed':
                    case 'atk':
                        this.player.debugAttackSpeedMultiplier = Math.max(0.25, Math.min(5, raw / 100));
                        break;
                    case 'movespeed':
                    case 'move':
                        this.player.debugMovementSpeedMultiplier = Math.max(0.25, Math.min(5, raw / 100));
                        this.player.getMovementSpeed();
                        break;
                    default:
                        return this.player.notify('Stats: level/lv, str, hp, attackspeed/atk, movespeed/move. Velocidades usam % (100 = normal).');
                }
                return this.player.notify(`DEV ${stat} atualizado para ${raw}. Use /devstats para conferir.`);
            }
