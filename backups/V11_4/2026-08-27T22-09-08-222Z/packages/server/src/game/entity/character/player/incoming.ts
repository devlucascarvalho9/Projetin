...
export default class Incoming {
        ...
        this.connection.onMessage(([packet, message]) => {
            ...
            try {
                switch (packet) {
                    ...
                    case Packets.Talent: {
                        return this.handleTalent(message);
                    }
                    case Packets.Pet: {
    ...
    private handleLogin(data: LoginPacket): void {
        ...
        switch (opcode) {
            ...
            case Opcodes.Login.Guest: {
                // When MongoDB is enabled, use a stable development guest so local
                // progress survives browser/server restarts. Registered accounts still
                // remain the recommended option for public/multi-user servers.
                if (!config.skipDatabase) {
                    const rawAddress = this.connection.address || 'localhost';
                    const normalized = rawAddress === '::1' || rawAddress === '127.0.0.1'
                        ? 'localhost'
                        : rawAddress.replace(/[^a-zA-Z0-9]/g, '_').slice(0, 22);
                    return this.database.guest(this.player, `guest_${normalized}`);
                }
    ...
    private handleEnchant(packet: EnchantPacket): void {
        ...
        if (packet.index !== undefined) packet.index = Utils.sanitizeNumber(packet.index);
        if (packet.shardIndex !== undefined) packet.shardIndex = Utils.sanitizeNumber(packet.shardIndex);
        ...
        switch (packet.opcode) {
            ...
            case Opcodes.Enchant.EssenceApply: {
                return this.world.enchanter.applyEssence(this.player, packet.previewId || '');
            }

            case Opcodes.Enchant.EssenceCancel: {
                return this.world.enchanter.cancelEssence(this.player, packet.previewId || '');
            }
        }
    }
    ...
    /** Handles Warrior passive-tree actions from the client. */
    private handleTalent(data: TalentPacket): void {
        switch (data.opcode) {
            case Opcodes.Talent.Sync: {
                return this.player.talents.sync();
            }
            case Opcodes.Talent.Toggle: {
                if (data.node) this.player.talents.toggle(data.node);
                return;
            }
            case Opcodes.Talent.Reset: {
                return this.player.talents.reset();
            }
            case Opcodes.Talent.Ascendancy: {
                if (data.ascendancy) this.player.talents.setAscendancy(data.ascendancy);
                return;
            }
        }
    }
