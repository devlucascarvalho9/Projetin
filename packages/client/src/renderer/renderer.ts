...
export default class Renderer {
    ...
    private drawEntity(entity: Entity): void {
        ...
        // The local player can be fully replaced by the KayKit renderer. We still
        // render its shadow and gameplay effects, but never draw the original
        // Kaetram body/paper-doll on top of the replacement character.
        const replaceLocalPlayer =
            entity === this.game.player &&
            Boolean((globalThis as typeof globalThis & { __kaetramKayKitReplaceLocalPlayer?: boolean }).__kaetramKayKitReplaceLocalPlayer);
        ...
        } else if ((entity as Character).hasActiveEffect()) this.drawEffects(entity as Character);

        context.restore();

        if (!replaceLocalPlayer) this.drawHealth(entity as Character);
