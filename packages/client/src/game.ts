...
export default class Game {
    ...
    public constructor(public app: App) {
        ...
        // OpenTibia integration bridge used by the standalone OT Skills / OT Maps UI.
        (globalThis as typeof globalThis & { __kaetramGame?: Game }).__kaetramGame = this;

        // Load the data-driven OT Skills / OT Maps interface without coupling 688 spells to the main bundle.
        if (!document.getElementById('ot-integration-script')) {
            const script = document.createElement('script');
            script.id = 'ot-integration-script';
            script.src = '/ot-integration-20260828-warrior-v2.js';
            script.defer = true;
    ...
    public castOtSpell(key: string): void {
        this.socket.send(Packets.Ability, { opcode: Opcodes.Ability.Use, key: `ot:${key}` });
    }

    public sendOtCommand(command: string): void {
        this.socket.send(Packets.Chat, [command]);
    }
    ...
    private openWorldMapSelector(): void {
        ...
        const destinations = [
            {
                id: -2,
                name: 'Cidade Ilustrada V11.2',
                biome: 'meadow' as BiomeId,
                biomeName: 'Cidade Principal',
                requirement: 'Teleporte /cidade',
                description: 'A cidade ilustrada preferida, movida para terreno totalmente vazio e equipada com 782 colisões próprias.'
            },
            {
                id: -3,
                name: 'Cripta V11.0',
            ...
            {
                id: -1,
                name: 'Cidade Estruturada V11.1',
                biome: 'meadow' as BiomeId,
                biomeName: 'Comparação',
                requirement: 'Teleporte /cidade2',
                description: 'Cidade alternativa mantida para comparação visual.'
            },
            {
                id: 0,
                name: 'Mudwich',
        ...
        const selectDestination = (id: number, biome: BiomeId) => {
            ...
            else if (id === -1) this.socket.send(Packets.Chat, ['/cidade2']);
    ...
    private openCraftForge(): void {
        ...
        for (const slot of Array.from(document.querySelectorAll<ForgeSlot>('#inventory-container > ul > li'))) {
            ...
            const domIndex = slot.querySelector<HTMLElement>('.item-slot')?.dataset.index;
            const fallbackIndex = Array.from(slot.parentElement?.children || []).indexOf(slot);
            const itemIndex = domIndex !== undefined ? Number(domIndex) : fallbackIndex;
