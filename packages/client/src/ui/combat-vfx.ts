...
export default class CombatVfx {
    ...
    public onHit(attacker: Character, target: Character, hit: HitLike): void {
        ...
        if (crit) this.spawnKenneyBurst(pos.x, pos.y, 'yellow', 0.62);
    }
    ...
    public skillPulse(kind: 'cleave' | 'whirlwind' | 'warcry'): void {
        ...
        if (kind === 'cleave') {
            ...
            this.spawnKenneyBurst(p.x, p.y, 'orange', 0.72);
            ...
            this.spawnKenneyBurst(p.x, p.y, 'red', 0.95);
        }
    }
...
`;
    ...
    private spawnKenneyBurst(x: number, y: number, kind: 'yellow' | 'orange' | 'red', scale = 1): void {
        const image = document.createElement('img');
        image.alt = '';
        image.src = kind === 'yellow'
            ? '/img/effects/arpg/kenney_burst_yellow.png'
            : kind === 'orange'
                ? '/img/effects/arpg/kenney_ring_orange.png'
                : '/img/effects/arpg/kenney_burst_red.png';
        Object.assign(image.style, {
            position: 'fixed', left: `${x}px`, top: `${y}px`, width: `${160 * scale}px`, height: `${160 * scale}px`,
            zIndex: '9984', pointerEvents: 'none', transform: 'translate(-50%,-50%) scale(.28) rotate(-8deg)',
            transformOrigin: '50% 50%', opacity: '.82', mixBlendMode: 'screen',
            filter: 'drop-shadow(0 0 8px rgba(255,172,55,.28))',
            transition: 'transform 260ms cubic-bezier(.15,.8,.2,1),opacity 320ms ease-out'
        });
        document.body.appendChild(image);
        requestAnimationFrame(() => {
            image.style.transform = 'translate(-50%,-50%) scale(1.08) rotate(8deg)';
            image.style.opacity = '0';
        });
        setTimeout(() => image.remove(), 360);
    }
