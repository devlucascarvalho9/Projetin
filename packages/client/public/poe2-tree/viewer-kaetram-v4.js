...
function nodeDisplay(key, node) {
  ...
  return {
    name: meta?.name || (key === state.overlay?.startNode ? 'GUERREIRO' : 'Passiva do Guerreiro'),
...
function shortestPathTo(target) {
  ...
  while (queue.length) {
    ...
    for (const next of talentLinks(current)) {
      ...
      if (meta?.ascendancyId && meta.ascendancyId !== state.selectedAscendancy) continue;
...
function typeName(key, n) {
  ...
  if (type === 'hub') return 'Hub do Guerreiro';
  if (type === 'ascendancyStart') return 'Início de Especialização';
  if (n?.ascendancyId) return 'Especialização';
  return 'Passiva';
}
...
function showTooltip(hit, sx, sy) {
  ...
  const y=Math.min(state.height-240, Math.max(10,sy+14));
...
function updateSearch() {
  ...
  if (q) {
    for (const [key,n] of Object.entries(state.data.nodes)) {
      ...
      const d=nodeDisplay(key,n);
      const hay = `${d.name} ${d.category} ${d.stats.join(' ')}`.toLocaleLowerCase('pt-BR');
      if (hay.includes(q)) state.searchMatches.add(key);
    }
  }
  const visibleCount=Object.entries(state.data.nodes).filter(([key,n])=>visibleNodePosition(n) && !n.isMastery && talentMeta(key)).length;
...
canvas.addEventListener('pointerup', e => {
  ...
  const meta=talentMeta(hit.key);
  if(!state.serverReady || !meta?.selectable) return;
  postToGame({ type:'kaetram-talents-toggle', node:hit.key });
});
...
ascendancySelect.addEventListener('change', () => { state.selectedAscendancy=ascendancySelect.value; updateAscendancyTransform(); updateSearch(); requestRender(); if (state.serverReady) postToGame({ type:'kaetram-talents-ascendancy', ascendancy:state.selectedAscendancy }); });
...
window.addEventListener('message', (event) => {
  ...
  if (message?.type !== 'kaetram-talents-sync' || !message.payload) return;
  const data=message.payload;
  state.selected=new Set(Array.isArray(data.selected)?data.selected:[state.overlay?.startNode || '47175']);
  state.serverReady=true;
  state.totalPoints=Number(data.totalPoints || 0);
  state.spentPoints=Number(data.spentPoints || 0);
  state.availablePoints=Number(data.availablePoints || 0);
  state.effects=data.effects || {};
  if (data.ascendancy) {
    state.selectedAscendancy=data.ascendancy;
    ascendancySelect.value=data.ascendancy;
...
async function boot() {
  try {
    resize();
    const [data, overlay, background, frame, groupBackground, skillsDisabled, warrior] = await Promise.all([
    ...
    state.data=data; state.overlay=overlay;
    ...
    postToGame({type:'kaetram-talents-ready'});
  } catch (error) {
    console.error(error);
