...
function visibleNonStartBounds() {
  ...
  for (const [key,n] of Object.entries(state.data.nodes)) {
    ...
    if (talentMeta(key)?.type === 'start') continue;
...
function orientWarriorSectionHorizontal() {
  const bounds = visibleNonStartBounds();
  ...
  for (const node of Object.values(state.data.nodes)) {
    ...
    const p = rotatePoint(node.x, node.y, cx, cy, angle);
    node.x = p.x; node.y = p.y;
  }

  for (const group of Object.values(state.data.groups || {})) {
    if (!Number.isFinite(group.x) || !Number.isFinite(group.y)) continue;
    const p = rotatePoint(group.x, group.y, cx, cy, angle);
    group.x = p.x; group.y = p.y;
  }

  for (const edge of state.data.edges || []) {
    if (Number.isFinite(edge.orbitX) && Number.isFinite(edge.orbitY)) {
      const p = rotatePoint(edge.orbitX, edge.orbitY, cx, cy, angle);
      edge.orbitX = p.x; edge.orbitY = p.y;
    }
  }

  const d = visibleNonStartBounds();
  const centerX = (d.minX + d.maxX) / 2;
  const baseY = d.maxY + Math.max(620, (d.maxY - d.minY) * 0.08);
  const spread = Math.min(820, Math.max(430, (d.maxX - d.minX) * 0.12));

  const left = state.data.nodes['kaetram_start_brutalidade'];
  const mid = state.data.nodes['kaetram_start_bastiao'];
  const right = state.data.nodes['kaetram_start_disciplina'];
  if (left) { left.x = centerX - spread; left.y = baseY; }
  if (mid) { mid.x = centerX; mid.y = baseY - 170; }
  if (right) { right.x = centerX + spread; right.y = baseY; }
}
...
async function boot(){
  try{
    ...
    orientWarriorSectionHorizontal();
