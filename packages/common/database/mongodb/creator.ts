...
export interface PlayerInfo {
    ...
    arpgSettings?: import('@kaetram/server/src/game/entity/character/player/player').ArpgPlayerSettings;
}
...
export default class Creator {
    ...
    /**
     * Saves the Warrior passive-tree selection.
     */

    public saveTalents(player: Player): void {
        let collection = this.database.collection('player_talents');

        this.updateCollection(collection, player.username, player.talents.serialize());
    }
    ...
    private updateCollection<S>(collection: Collection, username: string, data: S) {
        collection
            .updateOne({ username }, { $set: data as any }, { upsert: true })
            .then((result) => {
                if (!result)
    ...
    public static serialize(player: Player): PlayerInfo {
        return {
            ...
            arpgSettings: player.serializeArpgSettings(),
