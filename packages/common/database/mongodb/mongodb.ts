...
export default class MongoDB {
    ...
    /**
     * Development-friendly persistent guest login. Guests are keyed by a stable
     * local identifier supplied by the server (for example guest_localhost).
     * This bypasses password checks but still loads/saves the normal player
     * collections, preventing local testing progress from resetting to level 1.
     */
    public guest(player: Player, username: string): void {
        if (!this.hasDatabase()) {
        ...
        this.database
            ...
            .then((info) => {
                ...
                log.debug(`Creating persistent development guest ${username}.`);
