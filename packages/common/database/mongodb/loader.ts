...
import type { SerializedTalents } from '@kaetram/server/src/game/entity/character/player/talents';
...
export default class Loader {
    ...
    /**
     * Loads the Warrior passive-tree selection.
     */

    public loadTalents(player: Player): Promise<SerializedTalents | undefined> {
        return new Promise<SerializedTalents | undefined>((resolve) => {
            this.load(player.username, 'player_talents', (info: unknown) => {
                if (!info) return resolve(undefined);

                const [talents] = info as SerializedTalents[];
                resolve(talents);
            });
        });
    }
