...
export default class Item extends Entity {
    ...
    public constructor(
        ...
        // ARPG affixes are stored inside the existing enchantment payload so they
        // persist naturally in inventory/equipment MongoDB documents. Apply the
        // supported affixes to the real Kaetram equipment stats on load.
        this.applyArpgAffixes();

        if (this.data.plugin) this.loadPlugin();
    }

    /** Applies persistent ARPG/Essence affixes to the runtime equipment stats. */
    private applyArpgAffixes(): void {
        for (const enchantment of Object.values(this.enchantments)) {
            if (!['essence', 'arpg', 'unique'].includes(enchantment.source || '') || !enchantment.stat || !enchantment.value) continue;

            const value = enchantment.value;
            switch (enchantment.stat) {
                case 'strength':
                    this.bonuses.strength += value;
                    break;
                case 'accuracy':
                    this.bonuses.accuracy += value;
                    break;
                case 'slash':
                    this.attackStats.slash += value;
                    break;
                case 'crush':
                    this.attackStats.crush += value;
                    break;
                case 'stab':
                    this.attackStats.stab += value;
                    break;
                case 'magic':
                    this.attackStats.magic += value;
                    this.bonuses.magic += Math.max(1, Math.floor(value / 4));
                    break;
                case 'defense':
                    this.defenseStats.crush += value;
                    this.defenseStats.slash += value;
                    this.defenseStats.stab += value;
                    this.defenseStats.archery += Math.ceil(value * 0.6);
                    this.defenseStats.magic += Math.ceil(value * 0.4);
                    break;
                case 'attackSpeedPct':
                    this.attackRate = Math.max(250, Math.round(this.attackRate * (1 - value / 100)));
                    break;
                case 'movementSpeedPct':
                    if (this.itemType === 'boots') {
                        const base = this.movementModifier > 0 ? this.movementModifier : 1;
                        this.movementModifier = Math.max(0.55, base * (1 - value / 100));
                    }
                    break;
            }
        }
