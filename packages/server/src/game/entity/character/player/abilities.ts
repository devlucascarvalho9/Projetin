...
import OtSpellbook from './ot-spellbook';
...
export default class Abilities {
    private otSpellbook: OtSpellbook;
    ...
    public constructor(private player: Player) {
        this.otSpellbook = new OtSpellbook(player);
    }
    ...
    public load(info: SerializedAbility): void {
        ...
        // Starter Warrior kit. These are granted safely after database load so
        // existing characters keep all of their abilities and saves.
        for (const key of ['cleave', 'whirlwind', 'warcry'])
            if (!this.has(key)) this.add(key, 1, -1, true);
    ...
    public use(key: string): void {
        if (key.startsWith('ot:')) {
            this.otSpellbook.use(key.slice(3));
            return;
        }
