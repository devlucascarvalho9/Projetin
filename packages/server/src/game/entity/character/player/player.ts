...
export type ArpgSupportKey = 'area' | 'efficiency' | 'acceleration' | 'vitality';
export type ArpgSkillKey = 'cleave' | 'whirlwind' | 'warcry';
export type ArpgRaritySetting = 'normal' | 'magic' | 'rare' | 'unique';
export interface ArpgPlayerSettings {
    supports: Record<ArpgSkillKey, ArpgSupportKey[]>;
    lootFilter: {
        minRarity: ArpgRaritySetting;
        autoSellAtOrBelow: ArpgRaritySetting | 'off';
    };
}
...
export default class Player extends Character {
    ...
    public arpgSettings: ArpgPlayerSettings = {
        supports: { cleave: [], whirlwind: [], warcry: [] },
        lootFilter: { minRarity: 'normal', autoSellAtOrBelow: 'off' }
    };
    ...
    // V11.1 developer-panel overrides. These are intentionally session-only and
    // never written to character progression, so testing cannot corrupt a save.
    public debugCombatLevel = -1;
    public debugStrengthLevel = -1;
    public debugDexterityLevel = -1;
    public debugAgilityLevel = -1;
    public debugEvasionLevel = -1;
    public debugArcheryLevel = -1;
    public debugMagicLevel = -1;
    public debugMaxHitPoints = -1;
    public debugMaxMana = -1;
    public debugArmorBonus = 0;
    public debugCriticalChance = -1;
    public debugDamageMultiplier = 1;
    public debugAttackSpeedMultiplier = 1;
    public debugMovementSpeedMultiplier = 1;
    ...
    public async load(data: PlayerInfo): Promise<void> {
        ...
        this.loadArpgSettings(data.arpgSettings);
    ...
    /** Loads the Warrior passive-tree data before combat stats are synchronized. */
    public async loadTalents(): Promise<void> {
        this.talents.load(await this.database.loader?.loadTalents(this));
    }
    ...
    public getMovementSpeed(): number {
        ...
        // Passive tree movement bonuses lower the movement interval (lower is faster).
        speed = Math.max(40, Math.floor(speed / this.talents.getMovementSpeedMultiplier()));

        // AGI is a V11.2 development stat: 100 AGI gives roughly +20% movement speed.
        const agilityMovement = this.debugAgilityLevel > 0 ? Math.min(3, 1 + (this.debugAgilityLevel - 1) * 0.002) : 1;
        speed = Math.max(20, Math.floor(speed / agilityMovement));

        // Developer multiplier: 100% = normal, 200% = twice as fast.
        speed = Math.max(20, Math.floor(speed / Math.max(0.25, this.debugMovementSpeedMultiplier)));
    ...
    /** Loads persisted ARPG supports and loot preferences with safe defaults. */
    private loadArpgSettings(settings?: Partial<ArpgPlayerSettings>): void {
        const validSkills: ArpgSkillKey[] = ['cleave', 'whirlwind', 'warcry'];
        const validSupports: ArpgSupportKey[] = ['area', 'efficiency', 'acceleration', 'vitality'];
        const supports = settings?.supports || ({} as Record<ArpgSkillKey, ArpgSupportKey[]>);
        for (const skill of validSkills) {
            const selected = Array.isArray(supports[skill]) ? supports[skill] : [];
            this.arpgSettings.supports[skill] = selected.filter((support) => validSupports.includes(support)).slice(0, 2);
        }

        const rarities: ArpgRaritySetting[] = ['normal', 'magic', 'rare', 'unique'];
        const minRarity = settings?.lootFilter?.minRarity;
        const autoSell = settings?.lootFilter?.autoSellAtOrBelow;
        if (minRarity && rarities.includes(minRarity)) this.arpgSettings.lootFilter.minRarity = minRarity;
        if (autoSell === 'off' || (autoSell && rarities.includes(autoSell)))
            this.arpgSettings.lootFilter.autoSellAtOrBelow = autoSell;
    }

    public serializeArpgSettings(): ArpgPlayerSettings {
        return JSON.parse(JSON.stringify(this.arpgSettings)) as ArpgPlayerSettings;
    }

    public syncArpgSettings(): void {
        this.send(new CommandPacket({ command: `arpg-settings:${JSON.stringify(this.serializeArpgSettings())}` }));
    }

    public toggleArpgSupport(skill: string, support: string): string {
        const skillKey = skill as ArpgSkillKey;
        const supportKey = support as ArpgSupportKey;
        const compatibility: Record<ArpgSupportKey, ArpgSkillKey[]> = {
            area: ['cleave', 'whirlwind'],
            efficiency: ['cleave', 'whirlwind', 'warcry'],
            acceleration: ['cleave', 'whirlwind', 'warcry'],
            vitality: ['warcry']
        };
        if (!(skillKey in this.arpgSettings.supports)) return 'Skill inválida. Use cleave, whirlwind ou warcry.';
        if (!(supportKey in compatibility)) return 'Support inválido. Use area, efficiency, acceleration ou vitality.';
        if (!compatibility[supportKey].includes(skillKey)) return 'Esse Support não é compatível com esta Skill.';

        const list = this.arpgSettings.supports[skillKey];
        const existing = list.indexOf(supportKey);
        if (existing >= 0) {
            list.splice(existing, 1);
            this.syncArpgSettings();
            this.save();
            return `${supportKey} removido de ${skillKey}.`;
        }
        if (list.length >= 2) return `${skillKey} já possui 2 Supports. Remova um antes.`;
        list.push(supportKey);
        this.syncArpgSettings();
        this.save();
        return `${supportKey} vinculado a ${skillKey}.`;
    }

    public getArpgAbilityModifiers(skill: string): {
        manaMultiplier: number;
        cooldownMultiplier: number;
        radiusBonus: number;
        healMultiplier: number;
    } {
        const supports = this.arpgSettings.supports[skill as ArpgSkillKey] || [];
        ...
        for (const support of supports) {
            if (support === 'area') {
                ...
                manaMultiplier *= 1.15;
            } else if (support === 'vitality') {
                healMultiplier *= 1.65;
                manaMultiplier *= 1.15;
            }
        }
        return { manaMultiplier, cooldownMultiplier, radiusBonus, healMultiplier };
    }

    public setArpgLootFilter(minRarity: string): boolean {
        const rarities: ArpgRaritySetting[] = ['normal', 'magic', 'rare', 'unique'];
        if (!rarities.includes(minRarity as ArpgRaritySetting)) return false;
        this.arpgSettings.lootFilter.minRarity = minRarity as ArpgRaritySetting;
        this.syncArpgSettings();
        this.save();
        return true;
    }

    public setArpgAutoSell(rarity: string): boolean {
        const rarities: Array<ArpgRaritySetting | 'off'> = ['off', 'normal', 'magic', 'rare'];
        if (!rarities.includes(rarity as ArpgRaritySetting | 'off')) return false;
        this.arpgSettings.lootFilter.autoSellAtOrBelow = rarity as ArpgRaritySetting | 'off';
        this.syncArpgSettings();
        this.save();
        return true;
    }

    /** Starts periodic persistence for active characters during development. */
    public startAutosave(): void {
        if (this.autoSaveInterval) return;

        this.autoSaveInterval = setInterval(() => this.save(), 20_000);
    }
    ...
    public override getDefenseStats(): Stats {
        const stats = this.equipment.totalDefenseStats;
        if (this.debugArmorBonus <= 0) return stats;
        return {
            crush: stats.crush + this.debugArmorBonus,
            slash: stats.slash + this.debugArmorBonus,
            stab: stats.stab + this.debugArmorBonus,
            archery: stats.archery + this.debugArmorBonus,
            magic: stats.magic + this.debugArmorBonus
        };
    }
    ...
    public override getAccuracyBonus(): number {
        ...
        return this.getBonuses().accuracy + this.talents.getEffect('accuracyBonus');
    }
    ...
    public override getAccuracyLevel(): number {
        if (this.isMagic()) return this.debugMagicLevel > 0 ? this.debugMagicLevel : this.skills.get(Modules.Skills.Magic).level;
        if (this.isArcher()) return this.debugArcheryLevel > 0 ? this.debugArcheryLevel : this.skills.get(Modules.Skills.Archery).level;
        if (this.debugDexterityLevel > 0) return this.debugDexterityLevel;
    ...
    public override getStrengthLevel(): number {
        if (this.debugStrengthLevel > 0) return this.debugStrengthLevel;
    ...
    public override getArcheryLevel(): number {
        return this.debugArcheryLevel > 0 ? this.debugArcheryLevel : this.skills.get(Modules.Skills.Archery).level;
    }
    ...
    public override getDefenseLevel(): number {
        if (this.debugEvasionLevel > 0) return this.debugEvasionLevel;
        const level = this.skills.get(Modules.Skills.Defense).level;
        return Math.max(1, Math.round(level * this.talents.getDefenseLevelMultiplier()));
    }
    ...
    private getMagicLevel(): number {
        return this.debugMagicLevel > 0 ? this.debugMagicLevel : this.skills.get(Modules.Skills.Magic).level;
    }
    ...
    public override getSkillDamageLevel(): number {
        let level: number;
        if (this.isMagic()) level = this.getMagicLevel();
        else if (this.isArcher()) level = this.getArcheryLevel();
        else level = Math.max(1, Math.round(this.getStrengthLevel() * this.talents.getPhysicalDamageMultiplier()));
        return Math.max(1, Math.round(level * Math.max(0.25, this.debugDamageMultiplier)));
    }
    ...
    public override getDamageReduction(): number {
        ...
        reduction -= this.talents.getDamageReductionBonus();

        return Math.max(0.35, reduction);
    }
    ...
    public override getDamageType(): Modules.Hits {
        if (!this.isMagic() && !this.isArcher() && Utils.randomFloat(0, 100) <= (this.debugCriticalChance >= 0 ? this.debugCriticalChance : this.talents.getCriticalChance()))
            return Modules.Hits.Critical;
    ...
    public override hit(damage: number, attacker?: Character, aoe = 0, isThorns = false): void {
        // Some native recalculations can touch max HP between debug commands and combat.
        // Reassert the DEV maximum before damage so a hit can never collapse it back to the derived value.
        if (this.debugMaxHitPoints > 0 && this.hitPoints.getMaxHitPoints() !== this.debugMaxHitPoints)
            this.hitPoints.setMaxHitPoints(this.debugMaxHitPoints);
        super.hit(damage, attacker, aoe, isThorns);
    }

    public override getAttackRate(): number {
        let rate = this.equipment.getWeapon().attackRate;

        // Dualists mark status effect boosts attack speed by 200 milliseconds.
        if (this.status.has(Modules.Effects.DualistsMark)) rate -= 200;

        // Passive tree attack-speed bonuses lower the attack interval.
        rate = Math.floor(rate / this.talents.getAttackSpeedMultiplier());

        // AGI is also a small natural attack-speed accelerator while testing.
        const agilityAttack = this.debugAgilityLevel > 0 ? Math.min(2.5, 1 + (this.debugAgilityLevel - 1) * 0.0015) : 1;
        rate = Math.floor(rate / agilityAttack);

        // Developer multiplier: 100% = normal, 200% = twice as fast.
        rate = Math.floor(rate / Math.max(0.25, this.debugAttackSpeedMultiplier));
