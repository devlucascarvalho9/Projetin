...
export default class Skills {
    ...
    public sync(): void {
        ...
        // Update max hit points and mana. Developer HP override is session-only.
        const derivedMaxHitPoints = this.player.talents.applyMaxLife(Formulas.getMaxHitPoints(health.level));
        this.player.hitPoints.setMaxHitPoints(
            this.player.debugMaxHitPoints > 0 ? this.player.debugMaxHitPoints : derivedMaxHitPoints
        );
        if (this.player.hitPoints.getHitPoints() > this.player.hitPoints.getMaxHitPoints())
            this.player.hitPoints.setHitPoints(this.player.hitPoints.getMaxHitPoints());
        const derivedMaxMana = Formulas.getMaxMana(magic.level);
        this.player.mana.setMaxMana(this.player.debugMaxMana > 0 ? this.player.debugMaxMana : derivedMaxMana);
        if (this.player.mana.getMana() > this.player.mana.getMaxMana())
        ...
        if (this.player.talents.isLoaded()) this.player.talents.sync();
    }
    ...
    private handleExperience(
        ...
        if (newLevel) {
            ...
            if (type === Modules.Skills.Health)
                this.player.setHitPoints(this.player.talents.applyMaxLife(Formulas.getMaxHitPoints(level)));

            // Update the player's level if they have gained a level in a combat skill.
            this.sync();

            // Persist level-ups immediately so a quick server restart does not
            // revert progression between autosave ticks.
            this.player.save();
        }
    ...
    public getCombatLevel(): number {
        if (this.player.debugCombatLevel > 0) return this.player.debugCombatLevel;
