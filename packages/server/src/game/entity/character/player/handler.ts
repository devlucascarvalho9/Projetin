...
import { ARPG_META_ID, getItemSellValue, getMeta, rarityRank } from '../../../arpg/items';
...
export default class Handler {
    ...
    private handleInventoryAdd(slot: Slot): void {
        // ARPG Auto-Sell only touches items carrying our explicit metadata marker, so
        // quest rewards and legacy Kaetram equipment are never sold accidentally.
        const autoSell = this.player.arpgSettings.lootFilter.autoSellAtOrBelow;
        const metaEntry = slot.enchantments[ARPG_META_ID];
        if (slot.equippable && metaEntry?.source === 'arpg-meta' && autoSell !== 'off') {
            const meta = getMeta(slot.enchantments);
            if (meta.rarity !== 'unique' && rarityRank(meta.rarity) <= rarityRank(autoSell)) {
                const value = getItemSellValue(slot.key, slot.enchantments) * Math.max(1, slot.count);
                const soldName = slot.name || slot.key;
                const index = slot.index;
                const count = slot.count;
                this.player.inventory.remove(index, count);
                this.player.inventory.add(new Item('gold', -1, -1, false, value));
                this.player.notify(`Auto-Sell: ${soldName} vendido por ${value} ouro.`);
                return;
            }
        }
