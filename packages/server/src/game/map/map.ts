...
import mainCityCollisions from '../../../data/map/maincity-collisions.json';
...
export default class Map {
    ...
    private coordinateCollisions = new Set<number>(mainCityCollisions as number[]);
    ...
    public isColliding(x: number, y: number, player?: Player): boolean {
        ...
        // Check the collision at a specified index, including structure collisions
        // that are tied to exact map coordinates instead of globally reused tile IDs.
        return hasEntity || this.isCollisionIndex(index);
    }

    /**
     * Checks whether an exact map coordinate is reserved as a static structure collision.
     * This is used by towns where a visual tile can be reused as walkable somewhere else.
     */
    public isCoordinateCollision(x: number, y: number): boolean {
        return this.coordinateCollisions.has(this.coordToIndex(x, y));
    }
