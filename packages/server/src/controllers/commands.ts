...
import OtMapsData from '../../data/ot-maps.json';
import OtSpellsData from '../../data/ot-spells.json';
import {
    UNIQUE_KEYS,
    createLootEnchantments,
    createUniqueEnchantments,
    getRandomBaseKey
} from '../game/arpg/items';
import { getMapTier } from '../game/arpg/mobs';
...
const OtMaps = OtMapsData as unknown as OtMapEntry[];
const OtSpells = OtSpellsData as OtSpellEntry[];
...
export default class Commands {
    ...
    private async handlePlayerCommands(command: string, blocks: string[]): Promise<void> {
        switch (command) {
            ...
            case 'love':
            case 'cidade2':
            case 'cidadeestruturada': {
                // MAP-NORMAL: native 48x36 Love map. No HTML/CSS overlay.
                this.player.teleport(408, 1074, true, false, true);
                return this.player.notify('Mapa do Amor nativo — 48x36 tiles, escala normal.');
            }

            case 'carverna':
            case 'caverna':
            case 'cave': {
                // AUDIT-NATIVE: 80x64 Tiled cave with the original blocking layer preserved.
                this.player.teleport(517, 1088, true, false, true);
                return this.player.notify('Caverna Kaetram auditada — mapa Tiled nativo 80x64.');
            }

            case 'cidadeantiga':
            case 'oldcity': {
                this.player.teleport(191, 160, true, false, true);
                return this.player.notify('Teleportado para a cidade antiga do mundo original.');
            }

            case 'dungeon':
            case 'crypt': {
                // V11.0: first rot.js-generated KayKit crypt.
                this.player.teleport(804, 46, true, false, true);
                return this.player.notify('Cripta V11.0: elimine os esqueletos e avance ate o Crypt Lord. Use /cidade para sair.');
            }

            case 'otmap': {
                const query = blocks.join('_').trim().toLowerCase();

                if (!query)
                    return this.player.notify('Uso: /otmap <id ou nome>. Use /otmaps para ver os 25 mapas.');

                const normalized = query.replace(/\s+/g, '_');
                const map = OtMaps.find(
                    (entry) =>
                        entry.slug === normalized ||
                        String(entry.id) === normalized ||
                        entry.name.toLowerCase().replace(/[^a-z0-9]+/g, '_').includes(normalized)
                );

                if (!map)
                    return this.player.notify(`OT Map nao encontrado: ${query}. Use /otmaps.`);

                const [x, y] = map.spawn;
                this.player.teleport(x, y, true, false, true);
                return this.player.notify(`OT Map ${map.id}/25 — ${map.name} [${map.theme}]`);
            }

            case 'otmaps': {
                let page = parseInt(blocks.shift() || '1');
                if (!page || page < 1) page = 1;

                const pageSize = 5;
                const pages = Math.ceil(OtMaps.length / pageSize);
                page = Math.min(page, pages);
                const start = (page - 1) * pageSize;

                this.player.notify(`OT MAPS — pagina ${page}/${pages} — /otmap <nome>`);
                for (const map of OtMaps.slice(start, start + pageSize))
                    this.player.notify(`${map.id}. /otmap ${map.slug} — ${map.name}`);
            ...
            case 'otskills': {
                ...
                for (const spell of OtSpells)
                    categories.set(spell.category, (categories.get(spell.category) || 0) + 1);

                this.player.notify(`OT SKILLS: ${OtSpells.length} skills importadas do OTServBR.`);
                this.player.notify(
                    [...categories.entries()]
                        .sort((a, b) => b[1] - a[1])
                ...
                return this.player.notify('Abra o botao OT Skills no topo ou use /otcast <key>.');
            }

            case 'otcast': {
                const key = blocks.join('_').trim().toLowerCase();
                if (!key) return this.player.notify('Uso: /otcast <skill_key>. Veja as keys em OT Skills.');

                this.player.abilities.use(`ot:${key}`);
                return;
            }

            case 'teleportes':
            case 'tps': {
                this.player.notify('/amor ou /cidade — Mapa do Amor atual');
                this.player.notify('/carverna, /caverna ou /cave — Caverna Tiled atual');
                this.player.notify('/dungeon — Cripta procedural V11.0 / Crypt Lord');
                this.player.notify('=== OT MAPS IMPORTADOS (25) ===');

                // User requested every imported map to appear directly in /teleportes.
                for (const map of OtMaps)
                    this.player.notify(`${map.id}. /otmap ${map.slug} — ${map.name}`);

                this.player.notify('Use /otmap <nome> para teleportar. /otmaps [pagina] mostra a lista paginada.');
                return this.player.notify('Use /coords apos teleportar para confirmar o destino.');
            }

            case 'conteudo':
            case 'novidades': {
                this.player.notify('V11.x: KayKit animado • Skeletons KayKit • VFX Kenney • EasyStar AutoFarm • Cripta procedural • Cidade Estruturada.');
                this.player.notify('ARPG: Normal/Magic/Rare/Unique • ilvl • affixes T1-T5 • 24 bases • 3 Uniques • Essencias • Supports • Elites • Map Tiers.');
                this.player.notify(`OpenTibia: ${OtSpells.length} skills + ${OtMaps.length} mapas importados. /otskills /otmaps /teleportes`);
                this.player.notify('Comandos: /teleportes, /itemlist [pagina], /giveitem <key> [qtd] [raridade] [ilvl], /arpgloot <ilvl> <raridade>.');
                return this.player.notify('GM: F10 abre o painel LV/STR/HP/ATK SPD/MOVE SPD. /devstats mostra os overrides.');
            }

            // Development helper: normal players may spawn only our ARPG test gear/essences.
            case 'drop': {
                const key = (blocks.shift() || '').toLowerCase();
                let count = parseInt(blocks.shift() || '1');
                if (!key.startsWith('essence') && !key.startsWith('testforge') && !key.startsWith('arpg_') && !key.startsWith('unique_'))
                    return this.player.notify('Durante os testes, /drop aceita essências, testforge, arpg_ e unique_.');
                if (!(key in Items)) return this.player.notify(`Item de teste não encontrado: ${key}`);
                if (!count || count < 1) count = 1;
                count = Math.min(count, 25);
                this.world.entities.spawnItem(key, this.player.x, this.player.y, true, count, {}, this.player.username);
                return;
            }

            case 'support': {
                const skill = (blocks.shift() || '').toLowerCase();
                const support = (blocks.shift() || '').toLowerCase();
                if (!skill || !support)
                    return this.player.notify('Uso: /support [cleave|whirlwind|warcry] [area|efficiency|acceleration|vitality]');
                return this.player.notify(this.player.toggleArpgSupport(skill, support));
            }

            case 'supportlist': {
                const supports = this.player.arpgSettings?.supports || { cleave: [], whirlwind: [], warcry: [] };
                this.player.notify('SUPPORTS DISPONIVEIS: area, efficiency, acceleration, vitality.');
                this.player.notify(`Cleave (golpe em arco): ${supports.cleave?.join(', ') || 'nenhum'} | compativeis: area, efficiency, acceleration.`);
                this.player.notify(`Steelstorm/Whirlwind: ${supports.whirlwind?.join(', ') || 'nenhum'} | compativeis: area, efficiency, acceleration.`);
                return this.player.notify(`Warcry: ${supports.warcry?.join(', ') || 'nenhum'} | compativeis: efficiency, acceleration, vitality.`);
            }

            case 'lootfilter': {
                const rarity = (blocks.shift() || '').toLowerCase();
                if (!this.player.setArpgLootFilter(rarity))
                    return this.player.notify('Uso: /lootfilter [normal|magic|rare|unique]');
                return this.player.notify(`Loot Filter: coletar ${rarity} ou melhor.`);
            }

            case 'autosell': {
                const rarity = (blocks.shift() || '').toLowerCase();
                if (!this.player.setArpgAutoSell(rarity))
                    return this.player.notify('Uso: /autosell [off|normal|magic|rare]');
                return this.player.notify(rarity === 'off' ? 'Auto-Sell desativado.' : `Auto-Sell: vender automaticamente ${rarity} ou pior.`);
            }

            case 'maptier': {
                const tier = getMapTier(this.player.skills.getCombatLevel());
                return this.player.notify(`Tier ARPG estimado para seu nível atual: T${tier}. Elites e raridade aumentam em tiers maiores.`);
            }

            case 'arpgloot': {
                let level = parseInt(blocks.shift() || `${this.player.skills.getCombatLevel()}`);
                const rarity = (blocks.shift() || '').toLowerCase();
                if (!Number.isFinite(level)) level = this.player.skills.getCombatLevel();
                level = Math.max(1, Math.min(100, level));
                const valid = ['normal', 'magic', 'rare', 'unique'];
                const rolledRarity = valid.includes(rarity) ? rarity : 'rare';
                const tier = getMapTier(level);
                let key: string;
                let enchantments;
                if (rolledRarity === 'unique') {
                    key = UNIQUE_KEYS[Utils.randomInt(0, UNIQUE_KEYS.length - 1)];
                    enchantments = createUniqueEnchantments(key, level, tier);
                } else {
                    key = getRandomBaseKey(level);
                    enchantments = createLootEnchantments(level, rolledRarity as 'normal' | 'magic' | 'rare', tier);
                }
                this.world.entities.spawnItem(key, this.player.x, this.player.y, true, 1, enchantments, this.player.username);
                return this.player.notify(`Drop ARPG criado: ${rolledRarity.toUpperCase()} • ilvl ${level} • T${tier}.`);
            }

            case 'itemlist': {
                const keys = Object.keys(Items).filter((key) =>
                    key.startsWith('arpg_') ||
                    key.startsWith('unique_') ||
                    key.startsWith('essence') ||
                    key.startsWith('testforge')
                );
                const pageSize = 8;
                let page = parseInt(blocks.shift() || '1');
                ...
                this.player.notify(`Itens de teste — pagina ${page}/${pages}:`);
                for (const key of keys.slice((page - 1) * pageSize, page * pageSize)) {
                    const item = (Items as Record<string, { name?: string }>)[key];
                    this.player.notify(`${key} — ${item?.name || key}`);
                }
                return this.player.notify(`Use /iteminfo <key> para detalhes. /giveitem <key> [qtd] [raridade] [ilvl].`);
            }

            case 'iteminfo': {
                const key = (blocks.shift() || '').toLowerCase();
                if (!key) return this.player.notify('Uso: /iteminfo <key>. Ex.: /iteminfo arpg_worldbreaker');
                const item = (Items as Record<string, any>)[key];
                if (!item) return this.player.notify(`Item nao encontrado: ${key}. Use /itemlist [pagina].`);
                if (!key.startsWith('arpg_') && !key.startsWith('unique_') && !key.startsWith('essence') && !key.startsWith('testforge'))
                    return this.player.notify('O /iteminfo V11.3 lista apenas os itens novos/de teste do ARPG.');

                const compactStats = (stats?: Record<string, number>) =>
                    stats ? Object.entries(stats).filter(([, value]) => Number(value) !== 0).map(([name, value]) => `${name} ${Number(value) > 0 ? '+' : ''}${value}`).join(', ') : '';
                this.player.notify(`${item.name || key} [${key}]`);
                this.player.notify(`Tipo: ${item.type || 'objeto'}${item.weaponType ? ` / ${item.weaponType}` : ''} | Nivel base: ${item.level ?? 1} | Valor: ${item.price ?? 0}`);
                if (item.description) this.player.notify(String(item.description));
                const attack = compactStats(item.attackStats), defense = compactStats(item.defenseStats), bonuses = compactStats(item.bonuses);
                if (attack) this.player.notify(`Ataque: ${attack}`);
                if (defense) this.player.notify(`Defesa: ${defense}`);
                if (bonuses) this.player.notify(`Bonus: ${bonuses}`);
                const extras: string[] = [];
                if (item.attackRate) extras.push(`Attack rate ${item.attackRate}ms`);
                if (item.attackRange) extras.push(`Alcance ${item.attackRange}`);
                if (item.twoHanded) extras.push('Duas maos');
                if (item.movementModifier) extras.push(`Movimento x${(1 / Number(item.movementModifier)).toFixed(2)}`);
                if (item.stackable) extras.push(`Stack ${item.maxStackSize || 1}`);
                if (extras.length) this.player.notify(`Extra: ${extras.join(' | ')}`);
                return;
            }

            case 'giveitem': {
                if (!this.player.isAdmin()) return this.player.notify('Comando restrito a administrador/GM.');
                const key = (blocks.shift() || '').toLowerCase();
                const item = (Items as Record<string, { name?: string }>)[key];
                if (!item) return this.player.notify('Item nao encontrado. Use /itemlist [pagina].');
                if (!key.startsWith('arpg_') && !key.startsWith('unique_') && !key.startsWith('essence') && !key.startsWith('testforge'))
                    return this.player.notify('V11.1 /giveitem fica restrito aos itens novos/de teste do ARPG.');

                let count = parseInt(blocks.shift() || '1');
                count = Number.isFinite(count) ? Math.max(1, Math.min(25, count)) : 1;
                const rarityArg = (blocks.shift() || 'normal').toLowerCase();
                const rarities = ['normal', 'magic', 'rare', 'unique'];
                let itemLevel = parseInt(blocks.shift() || `${this.player.skills.getCombatLevel()}`);
                itemLevel = Number.isFinite(itemLevel) ? Math.max(1, Math.min(100, itemLevel)) : 1;

                let enchantments = {};
                if (key.startsWith('unique_')) enchantments = createUniqueEnchantments(key, itemLevel, getMapTier(itemLevel));
                else if (key.startsWith('arpg_')) {
                    const rarity = rarities.includes(rarityArg) ? rarityArg : 'normal';
                    enchantments = createLootEnchantments(itemLevel, rarity as 'normal' | 'magic' | 'rare', getMapTier(itemLevel));
                }

                const created = new Item(key, -1, -1, false, count, enchantments, this.player.username);
                const added = this.player.inventory.add(created);
                if (added < 1) return this.player.notify('Inventario cheio: o item NAO foi criado. Libere um slot e tente novamente.');
                return this.player.notify(`Adicionado ao INVENTARIO: ${item.name || key} x${added} • ilvl ${itemLevel}.`);
            }

            case 'devheal': {
                if (!this.player.isAdmin()) return this.player.notify('Comando restrito a administrador/GM.');
                this.player.hitPoints.setHitPoints(this.player.hitPoints.getMaxHitPoints());
                this.player.mana.setMana(this.player.mana.getMaxMana());
                return this.player.notify(`Curado: HP ${this.player.hitPoints.getHitPoints()}/${this.player.hitPoints.getMaxHitPoints()} • Mana ${this.player.mana.getMana()}/${this.player.mana.getMaxMana()}.`);
            }

            case 'devstats': {
                if (!this.player.isAdmin()) return this.player.notify('Comando restrito a administrador/GM.');
                const lv = this.player.debugCombatLevel > 0 ? this.player.debugCombatLevel : this.player.skills.getCombatLevel();
                const str = this.player.debugStrengthLevel > 0 ? this.player.debugStrengthLevel : this.player.skills.get(Modules.Skills.Strength).level;
                const dex = this.player.debugDexterityLevel > 0 ? this.player.debugDexterityLevel : this.player.skills.get(Modules.Skills.Accuracy).level;
                const eva = this.player.debugEvasionLevel > 0 ? this.player.debugEvasionLevel : this.player.skills.get(Modules.Skills.Defense).level;
                this.player.notify(`DEV CORE — LV ${lv} | HP ${this.player.hitPoints.getMaxHitPoints()} | Mana ${this.player.mana.getMaxMana()} | STR ${str} | DEX ${dex} | EVA ${eva}.`);
                this.player.notify(`DEV EXTRA — AGI ${this.player.debugAgilityLevel > 0 ? this.player.debugAgilityLevel : 1} | ARCH ${this.player.debugArcheryLevel > 0 ? this.player.debugArcheryLevel : this.player.skills.get(Modules.Skills.Archery).level} | MAGIC ${this.player.debugMagicLevel > 0 ? this.player.debugMagicLevel : this.player.skills.get(Modules.Skills.Magic).level} | ARMOR +${this.player.debugArmorBonus}.`);
                return this.player.notify(`DEV MULT — CRIT ${this.player.debugCriticalChance >= 0 ? this.player.debugCriticalChance : 'normal'}% | DANO ${Math.round(this.player.debugDamageMultiplier * 100)}% | ATK ${Math.round(this.player.debugAttackSpeedMultiplier * 100)}% | MOVE ${Math.round(this.player.debugMovementSpeedMultiplier * 100)}%.`);
            }

            case 'devset': {
                if (!this.player.isAdmin()) return this.player.notify('Comando restrito a administrador/GM.');
                const stat = (blocks.shift() || '').toLowerCase();
                if (stat === 'reset') {
                    this.player.debugCombatLevel = -1;
                    this.player.debugStrengthLevel = -1;
                    this.player.debugDexterityLevel = -1;
                    this.player.debugAgilityLevel = -1;
                    this.player.debugEvasionLevel = -1;
                    this.player.debugArcheryLevel = -1;
                    this.player.debugMagicLevel = -1;
                    this.player.debugMaxHitPoints = -1;
                    this.player.debugMaxMana = -1;
                    this.player.debugArmorBonus = 0;
                    this.player.debugCriticalChance = -1;
                    this.player.debugDamageMultiplier = 1;
                    this.player.debugAttackSpeedMultiplier = 1;
                    this.player.debugMovementSpeedMultiplier = 1;
                    this.player.skills.sync();
                    this.player.getMovementSpeed();
                    this.player.combat.updateLoop();
                    return this.player.notify('Overrides DEV resetados para os valores normais do personagem.');
                }

                const raw = Number(blocks.shift());
                if (!Number.isFinite(raw)) return this.player.notify('Uso: /devset [level|hp|mana|str|dex|agi|evasion|archery|magic|armor|crit|damage|attackspeed|movespeed] <valor>.');
                switch (stat) {
                    case 'level': case 'lv': this.player.debugCombatLevel = Math.max(1, Math.min(999, Math.floor(raw))); break;
                    case 'str': case 'strength': this.player.debugStrengthLevel = Math.max(1, Math.min(999, Math.floor(raw))); break;
                    case 'dex': case 'dexterity': case 'accuracy': this.player.debugDexterityLevel = Math.max(1, Math.min(999, Math.floor(raw))); break;
                    case 'agi': case 'agility': this.player.debugAgilityLevel = Math.max(1, Math.min(999, Math.floor(raw))); break;
                    case 'evasion': case 'eva': case 'defense': this.player.debugEvasionLevel = Math.max(1, Math.min(999, Math.floor(raw))); break;
                    case 'archery': case 'arch': this.player.debugArcheryLevel = Math.max(1, Math.min(999, Math.floor(raw))); break;
                    case 'magic': case 'mag': this.player.debugMagicLevel = Math.max(1, Math.min(999, Math.floor(raw))); break;
                    case 'hp': case 'life':
                        this.player.debugMaxHitPoints = Math.max(1, Math.min(999_999, Math.floor(raw)));
                        this.player.hitPoints.setMaxHitPoints(this.player.debugMaxHitPoints);
                        this.player.hitPoints.setHitPoints(this.player.debugMaxHitPoints);
                        break;
                    case 'mana':
                        this.player.debugMaxMana = Math.max(1, Math.min(999_999, Math.floor(raw)));
                        this.player.mana.setMaxMana(this.player.debugMaxMana);
                        this.player.mana.setMana(this.player.debugMaxMana);
                        break;
                    case 'armor': case 'armour': this.player.debugArmorBonus = Math.max(0, Math.min(9999, Math.floor(raw))); break;
                    case 'crit': case 'critical': this.player.debugCriticalChance = Math.max(0, Math.min(100, raw)); break;
                    case 'damage': case 'dmg': this.player.debugDamageMultiplier = Math.max(0.25, Math.min(10, raw / 100)); break;
                    case 'attackspeed': case 'atk': this.player.debugAttackSpeedMultiplier = Math.max(0.25, Math.min(5, raw / 100)); break;
                    case 'movespeed': case 'move': this.player.debugMovementSpeedMultiplier = Math.max(0.25, Math.min(5, raw / 100)); break;
                    default: return this.player.notify('Stats: level, hp, mana, str, dex, agi, evasion, archery, magic, armor, crit, damage, attackspeed, movespeed.');
                }
                this.player.skills.sync();
                this.player.getMovementSpeed();
                this.player.combat.updateLoop();
                return this.player.notify(`DEV ${stat} = ${raw}. Use /devstats para conferir.`);
            }
